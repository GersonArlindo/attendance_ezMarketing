...
if(!defined('db_file')) define('db_file','C:\xampp\htdocs\asistencia_empleados\db/attendance_db.db');
...
class DBConnection extends SQLite3
{
    ...
    function __construct(){
        ...
        $this->exec("CREATE TABLE IF NOT EXISTS `employee_list` (
            ...
            FOREIGN KEY (`campaign_id`) REFERENCES `campaign_list`(`campaign_id`) ON DELETE CASCADE
        ) ");
        ...
        $this->exec("CREATE TABLE IF NOT EXISTS `hours_list` (
            `hour_id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
            `employee_id` INTEGER NOT NULL,
            `hour` TIMESTAMP NOT NULL Default CURRENT_TIMESTAMP,
            `accumulated_time` TIMESTAMP NOT NULL Default CURRENT_TIMESTAMP,
            `total_time` TIMESTAMP NOT NULL Default CURRENT_TIMESTAMP,
            `hours_breack` TIMESTAMP NOT NULL Default CURRENT_TIMESTAMP,
            `date_created` TIMESTAMP NOT NULL Default CURRENT_TIMESTAMP,
            FOREIGN KEY (`employee_id`) REFERENCES `employee_list`(`employee_id`) ON DELETE CASCADE
        ) ");

        $this->exec("CREATE TABLE IF NOT EXISTS `campaign_list` (
